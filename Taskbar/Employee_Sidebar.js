// Employee_Sidebar.js
// Handles: sidebar collapse/expand, employee name+email loading
// Called by each Employee page after the sidebar HTML is injected
...
function _initCollapseToggle() {
    ...
    const saved = localStorage.getItem('employeeSidebarCollapsed') === 'true';
    ...
    toggleBtn.addEventListener('click', () => {
        ...
        localStorage.setItem('employeeSidebarCollapsed', !isCollapsed);
    });
}
...
async function _loadSidebarUser() {
    ...
    try {
        // Pulls first_name, last_name, email from users table (employee role)
        ...
        if (response.ok) {
            ...
            nameEl.textContent  = `${data.first_name || ''} ${data.last_name || ''}`.trim() || 'Employee';
        ...
        console.error('Employee sidebar user fetch failed:', err);
        nameEl.textContent  = 'Employee';
        emailEl.textContent = '';
    }
}
